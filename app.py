...
def get_upcoming_matches():
    ...
    for fn in os.listdir(DATA_DIR):
        ...
        # Must be a list of match objects
        ...
        for m in data:
            ...
            # pick whichever time field exists
            time_str = (
                m.get("utcDate")
                or m.get("kickoff")
                or m.get("start")
...
def find_match_id(slug: str):
    """Case-insensitive lookup of match_id by Flashscore slug."""
    for fn in os.listdir(DATA_DIR):
        if fn == "leagues.json" or not fn.endswith(".json"):
            continue
        recs = json.load(open(os.path.join(DATA_DIR, fn), encoding="utf-8"))
        if isinstance(recs, list):
            for m in recs:
                if isinstance(m, dict) and m.get("slug", "").lower() == slug.lower():
                    return m.get("id") or m.get("match_id")
    return None

@app.get("/score/{identifier}")
def get_live_score(identifier: str):
    # Resolve numeric match_id
